...
class Sketch {
    ...
    setupGUI() {
        ...
        const boneRanges = {
            'shoulderL': { x: [-50, 180], y: [-90, 90], z: [-90, 90] },
            'upper_armL': { x: [-50, 180], y: [-90, 90], z: [-90, 90] },
            'forearmL': { x: [-10, 150], y: [-90, 90], z: [-90, 90] },
            'handL': { x: [-70, 80], y: [-30, 20], z: [-20, 30] },
            'shoulderR': { x: [-50, 180], y: [-90, 90], z: [-90, 90] },
            'upper_armR': { x: [-50, 180], y: [-90, 90], z: [-90, 90] },
            'forearmR': { x: [-10, 150], y: [-90, 90], z: [-90, 90] },
            'handR': { x: [-70, 80], y: [-30, 20], z: [-20, 30] }
        ...
        bonesToControl.forEach(boneName => {
            if (this.bones[boneName]) {
                ...
                folder.add(controls, 'rotationX', boneRanges[boneName].x[0], boneRanges[boneName].x[1], 0.01).name('Rotation X').onChange(value => {
                    this.applyRelativeRotation(boneName, 'x', THREE.MathUtils.degToRad(value));
                });
                folder.add(controls, 'rotationY', boneRanges[boneName].y[0], boneRanges[boneName].y[1], 0.01).name('Rotation Y').onChange(value => {
                    this.applyRelativeRotation(boneName, 'y', THREE.MathUtils.degToRad(value));
                });
                folder.add(controls, 'rotationZ', boneRanges[boneName].z[0], boneRanges[boneName].z[1], 0.01).name('Rotation Z').onChange(value => {
                    this.applyRelativeRotation(boneName, 'z', THREE.MathUtils.degToRad(value));
                });
            }
        });
    }
    ...
    addObjects() {
        this.material = new THREE.ShaderMaterial({
            ...
            uniforms: {
                ...
                uvRate1: { value: new THREE.Vector2(1, 1) },
                uTexture: { value: new THREE.TextureLoader().load(texture01) },
                boneMatrices: { value: [] }, // For skinning
            },
            vertexShader: vertex,
            fragmentShader: fragment,
        });
    ...
    async addModel() {
        try {
            const response = await fetch('../../public/models/ManAminate.glb.json'); // Adjust path
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const restPoseData = await response.json();
                ...
                (gltf) => {
                    ...
                    this.model.traverse(o => {
                        ...
                        if (o.isBone) {
                            ...
                            // Store rest pose matrix
                            this.restPoseMatrices[o.name] = new THREE.Matrix4();
                            const restMatrix = this.findRestPoseMatrix(o.name, restPoseData.scene.children);
                            if (restMatrix) {
                                this.restPoseMatrices[o.name].fromArray(restMatrix);
                                o.matrix.copy(this.restPoseMatrices[o.name]);
                                o.matrixWorldNeedsUpdate = true;
                            } else {
                                console.warn(`Rest pose matrix not found for bone: ${o.name}`);
                            }
                        }
                        console.log(o.name, "x :" + o.rotation.x, "y :" + o.rotation.y, "z :" + o.rotation.z);
                    });
            ...
            console.error('Error loading rest pose JSON:', error);
        }
    }

    findRestPoseMatrix(boneName, children) {
        for (let child of children) {
            if (child.name === boneName) {
                return child.matrix;
            }
            if (child.children) {
                const result = this.findRestPoseMatrix(boneName, child.children);
    ...
    applyRelativeRotation(boneName, axis, targetAngle) {
        const bone = this.bones[boneName];
        if (!bone || !this.restPoseMatrices[boneName]) return;

        // Decompose rest pose matrix to get original rotation
        const restMatrix = this.restPoseMatrices[boneName].clone();
        const restPosition = new THREE.Vector3();
        const restQuaternion = new THREE.Quaternion();
        const restScale = new THREE.Vector3();
        restMatrix.decompose(restPosition, restQuaternion, restScale);

        // Convert current rotation to quaternion
        const currentEuler = bone.rotation.clone();
        const currentQuaternion = new THREE.Quaternion().setFromEuler(currentEuler);

        // Calculate target quaternion based on GUI input
        const targetEuler = currentEuler.clone();
        targetEuler[axis] = targetAngle;
        const targetQuaternion = new THREE.Quaternion().setFromEuler(targetEuler);

        // Apply relative rotation (difference from rest pose)
        const relativeQuaternion = targetQuaternion.multiply(restQuaternion.invert());
        bone.quaternion.copy(relativeQuaternion);
    ...
    render() {
        ...
        if (this.model && this.model.skeleton) {
            const boneMatrices = this.model.skeleton.boneMatrices;
            this.material.uniforms.boneMatrices.value = boneMatrices;
        }
