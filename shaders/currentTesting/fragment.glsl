...
void main() {
    ...
    float diff = dot(vec3(1.),vWorldNormal);

    float phi = acos(vWorldNormal.y);
    float angle = atan(vWorldNormal.x, vWorldNormal.z);

    float fresnel = dot(cameraPosition, vWorldNormal);
    // float fresnel = dot(vWorldPos, vWorldNormal);
    fresnel =  fresnel * fresnel / 2.5;

    vec2 newFakeUV = vec2((angle / PI)/(2. / PI), phi / PI);
    newFakeUV = abs(newFakeUV);
    
    float diffPlusOne = dot(vec3(1.),vWorldNormal);
    float diffMinsOne = dot(vec3(-1., 0., 1.),vWorldNormal);
